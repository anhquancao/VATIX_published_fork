...
class MultiInceptionMetrics(Metric):
    ...
    def __init__(self, device, compute_manifold=False, num_classes=1000, num_inception_chunks=10, manifold_k=3,
                 model="inception", use_precomputed_feat="", **kwargs):
        ...
        self.use_precomputed_feat = use_precomputed_feat
        ...
        if model == "inception":
            ...
            class _Dinov2(nn.Module):
                ...
                def preprocess(self, images):
                    """ convert from {(size, size), [-1, 1], float32} --> {(224, 224), N(0, 1), float32} """
                    images = images.view(-1, 3, images.size(-2), images.size(-1))
                    images = torch.stack([self.t(img) for img in images])
                    return images

                def forward(self, x) -> Any:
                    out = self.model(self.preprocess(x)).last_hidden_state[:, 0]
            ...
            class I3D(nn.Module):
                ...
                def preprocess(video: Tensor) -> Tensor:
                    ...
                    # interpolate the size to i3d training data
                    video = torch.nn.functional.interpolate(video, (224, 224))
                ...
                def forward(self, x):
                    ...
                    out = self.model(self.preprocess(x), rescale=False, resize=False, return_features=True)
                    return out, torch.tensor([out.size(0), 10])
    ...
    def fid(self, real_features, fake_features):
        ...
        if self.use_precomputed_feat != "":
            print("Use precomputed Features")
            loaded_data = torch.load(self.use_precomputed_feat, weights_only=True)
    ...
    def compute(self) -> dict:
        ...
        # Gather all the tensor on the main process
        ...
        idx = torch.randperm(real_features.size(0))  # shuffle the images!
        real_features = real_features[idx]
        fake_features = fake_features[idx]

        output_metrics["FVD" if self.model_name == "i3d" else "FID"] = self.fid(real_features, fake_features)
